...
class UbahStatusOtpPembeliActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_ubah_status_otp_pembeli)
        ...
        val menuListener = object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                etStatus.setText(users.checkOtp)
                ...
                if (validStatus) {
                    if (status == "active" || status == "inactive"){
                        ...
                        val menuListener = object : ValueEventListener {
                            override fun onDataChange(dataSnapshot: DataSnapshot) {
                                val users = dataSnapshot.getValue(Users::class.java)!!
                                val usersUpdate = Users(users.idUsers, users.username, users.kelamin, users.alamat, users.email, users.photoProfil, users.noTelp, users.jumlahTransaksi, users.accessLevel, users.token, users.status, status)
                                reference.setValue(usersUpdate).addOnCompleteListener {
                                    if (it.isSuccessful){
                                        alertDialog("BERHASIL!", "Status OTP akun berhasil di ubah!", true)
                                        checkClick = true
                                    } else{
                                        alertDialog("Gagal Mengubah Status OTP!", "${it.exception?.message}", false)
                                        checkClick = true
                                    }
                                }
                            }
                        ...
                        alertDialog("GAGAL!", "Status OTP harus active/inactive!", false)
                        checkClick = true
                    }
                } else {
                    loadingBar(1000)
                    alertDialog("GAGAL!", "Status OTP akun gagal di ubah!", false)
                    checkClick = true
                }
            } else{
    ...
    private fun showInternetDialog() {
        ...
        alertDialog.apply {
            ...
            setPositiveButton(
                ...
                DialogInterface.OnClickListener { dialogInterface, i ->
                    dialogInterface.dismiss()
                    if (!isConnected(this@UbahStatusOtpPembeliActivity)){
                        showInternetDialog()
                    }
                })
        }
    ...
    private fun isConnected(contextActivity: UbahStatusOtpPembeliActivity): Boolean {
