    ...
    private fun updateData(changeImage: Boolean){
        ...
        if (changeImage == false){
            ...
            val menuListener = object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    val customers = dataSnapshot.getValue(Customers::class.java)!!
                    val customersUpdate = Customers(userIdentity.uid, nama, kelamin, customers.alamat, email, customers.photoProfil, telepon, customers.jumlahTransaksi, customers.accessLevel, customers.token, customers.status, customers.checkOtp)
            ...
            refImage.putFile(imageUri).addOnSuccessListener {
                ...
                refImage.downloadUrl.addOnSuccessListener { it1 ->
                    ...
                    val menuListener = object : ValueEventListener {
                        override fun onDataChange(dataSnapshot: DataSnapshot) {
                            val customers = dataSnapshot.getValue(Customers::class.java)!!
                            val customersUpdate = Customers(userIdentity.uid, nama, kelamin, customers.alamat, email, downloadUrl.toString(), telepon, customers.jumlahTransaksi, customers.accessLevel, customers.token, customers.status, customers.checkOtp)
                            referen.setValue(customersUpdate).addOnCompleteListener {
                                if (it.isSuccessful){
                                } else{
                                    alertDialog("Gagal Mengubah Data Pribadi!", "${it.exception?.message}", false)
